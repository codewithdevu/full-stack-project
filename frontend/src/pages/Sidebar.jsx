...
const Sidebar = () => {
    ...
    return (
        <aside className="fixed left-0 top-16 w-64 h-[calc(100vh-16px)] bg-slate-950/45 border-r border-slate-900/80 backdrop-blur-xl p-4 hidden lg:flex flex-col justify-between select-none z-30">
                ...
                {sections.map((section, sectionIdx) => (
                    <div key={sectionIdx} className="space-y-2">
                                ...
                                >
                                    {({ isActive }) => (
                                        <div className={`
                                            ...
                                            ${isActive 
                                                ? "bg-linear-to-r from-indigo-500/10 to-transparent text-indigo-200 border-l-2 border-indigo-500 shadow-sm shadow-indigo-500/5" 
                        ...
                        {sectionIdx < sections.length - 1 && (
                            <hr className="border-slate-900/60 my-4 mx-2" />
                        )}
                    </div>
                ))}
            </div>
            ...
            <div className="border-t border-slate-900/80 pt-4 px-3 flex flex-col gap-1">
                <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-[10px] text-slate-500 font-medium">HLS Transcoding Node Online</span>
                </div>
