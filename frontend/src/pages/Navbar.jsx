...
import { useNavigate, Link, useLocation } from "react-router-dom"; // 🟢 ADDED: useLocation for strict auth path masking
...
const Navbar = () => {
    ...
    const location = useLocation(); // 🟢 INITIALIZE LOCATION RADAR

    // 🟢 AUTH SCENE INTERCEPT GUARD:
    // If the active viewport hits auth layouts, completely unmount navbar layer instantly
    ...
    const handleSearchSubmit = (e) => {
        e.preventDefault();
        // 🟢 SAFETY INPUT GUARD: Prevent white space and blank query submissions
        if (searchQuery.trim()) {
    ...
    useEffect(() => {
        const fetchCurrentUser = async () => {
            try {
                ...
                console.log("Navbar: User session context offline");
                setUser(null);
            }
        };

        fetchCurrentUser();
    }, [location.pathname]); // 🟢 RE-FETCH ON ROUTE CHANGED: Updates profile picture instantly after user logs in
            ...
            {isMobileSearchOpen ? (
                <div className="flex items-center w-full gap-2 animate-in fade-in duration-200">
                    <button
                        ...
                        className="p-2 text-slate-400 hover:text-slate-200 bg-slate-900/40 border border-slate-800/80 rounded-xl outline-none"
                    >
                    ...
                    >
                        <div className="flex-1 flex items-center pl-3 pr-2 py-2.5">
                            <input
                                ...
                                className="w-full bg-transparent text-slate-200 placeholder-slate-500 border-0 outline-none text-xs font-medium focus:ring-0 focus:outline-none"
                                value={searchQuery}
                                onChange={(e) => setSearchQuery(e.target.value)}
                            />
                        </div>
                        <button type="submit" className="bg-slate-900 px-4 py-2.5 text-slate-400 border-l border-slate-800 outline-none active:scale-95 transition-transform">
                ...
                <>
                    ...
                    <Link
                        to="/"
                        className="flex items-center gap-2 hover:opacity-90 transition-opacity shrink-0 outline-none"
                    >
                    ...
                    {/* 2. Desktop Integrated Search Bar */}
                    <form
                        onSubmit={handleSearchSubmit}
                    ...
                    <div className="flex items-center gap-2 xs:gap-3 shrink-0">
                        ...
                        <button type="button" className="relative p-2 rounded-xl text-slate-400 hover:text-slate-200 hover:bg-slate-900/60 border border-transparent hover:border-slate-800/50 transition-all duration-300 group outline-none">
                        ...
                        <div
                            ...
                            className="relative w-8 h-8 xs:w-8.5 xs:h-8.5 rounded-full p-[1.5px] cursor-pointer bg-linear-to-tr from-slate-800 to-slate-900 hover:from-indigo-500 hover:to-pink-500 transition-all duration-300 shadow-md shadow-slate-950/50 group shrink-0"
                        >
